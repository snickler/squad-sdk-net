...
### Issue #37 Upstream Dev Sync — Teams Adapter Security (2026-04)

- After PR #45, the only remaining `src/**/*.cs` analogue from upstream `dev` HEAD `ec19f5e` was Teams adapter hardening.
- Ported identity-scoped token cache files hashed by `tenantId:clientId`, legacy cache migration from `teams-tokens.json`, refresh-token preservation, stale-token purge on permanent auth failures, and additive `ICommunicationAdapter.LogoutAsync()` support.
- Upstream `a69c527` was already partially covered by existing Graph ID encoding; the remaining Windows/non-Windows token permission hardening was added here.
- Upstream `85a3146`, `e49d9c9`, `c16a84b`, `f50bbc8`, and `3579a23` had no direct .NET source analogue in this repo after PR #45.
- Validation: `dotnet build src\\Squad.SDK.NET\\Squad.SDK.NET.csproj -p:NoWarn=NU1507` and `dotnet test tests\\Squad.SDK.NET.Tests\\Squad.SDK.NET.Tests.csproj -p:NoWarn=NU1507` both pass locally; plain solution restore still trips NU1507 because this machine exposes two package sources.

### Issue #37 Follow-up — Refresh Token Validation (2026-04)

- Tightened LoadTokens to reject empty or whitespace-only refresh tokens so cached token validation matches the adapter's runtime string.IsNullOrWhiteSpace(...) guard and avoids pointless refresh attempts.
## Learnings
